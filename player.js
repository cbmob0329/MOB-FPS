// player.js  v11
// ・弾は TA1～TA4.png で描画（?v=11 でキャッシュ破壊）
// ・銃口オフセットを見直し（顔から出ない）
// ・グレネードは必ず地面に落ちて爆発
...
export const GROUND_Y = 460; // 地面ライン
...
export class Bullet {
  ...
  step(dt) {
    ...
    // グレはしっかり落とす
    if (this.kind === 'grenade') this.vy += 980 * dt;
  ...
  draw(ctx, camX) {
    ...
    if (img && img.ok) ctx.drawImage(img.img, sx - (w>>1), sy - (h>>1), w, h);
      ...
      ctx.beginPath(); ctx.arc(sx, sy, (this.kind==='grenade')?4:2.5, 0, Math.PI*2); ctx.fill();
      ctx.restore();
    }
  }
}
...
export class MuzzleFlash {
  constructor({ x, y, dir = 1, life = 0.08 }) { this.x=x; this.y=y; this.dir=dir; this.life=life; this.t=0; this.dead=false; }
  step(dt){ this.t += dt; if (this.t >= this.life) this.dead = true; }
  draw(ctx, camX) {
    const a = 1 - (this.t/this.life);
    ctx.save(); ctx.globalAlpha = 0.6 * a; ctx.fillStyle = '#fff7ba';
    const p1x = this.x - camX, p1y = this.y, len = 10;
    ctx.beginPath();
    ctx.moveTo(p1x, p1y);
...
export class Explosion {
  constructor({ x, y, r = 60, dmg = 100, life = 0.35 }) { this.x=x; this.y=y; this.r=r; this.dmg=dmg; this.life=life; this.t=0; this.dead=false; }
  step(dt){ this.t += dt; if (this.t >= this.life) this.dead = true; }
  draw(ctx, camX){
    const a = 1 - (this.t / this.life);
...
export class Player {
  constructor(x, y) {
    ...
    this.weapons = {
      ...
      gl:    { dmg:50, rate:1.00, ammoMax:3,  ammo:3,  reload:5.0, speed:520, range:0.9 },
    ...
    // 銃口オフセット（右向き基準／左上原点）— 顔から出ないよう前方寄りに調整
    this.muzzle = {
      rifle: { x: 44, y: 30 },
      smg:   { x: 43, y: 30 },
      gl:    { x: 40, y: 33 },
  ...
  getMuzzlePos(kind){
    ...
    const off = this.muzzle[kind] || {x:40,y:30};
    // x は「描画左端」と一致している実装なので、そのまま左右でミラー
  ...
  tryFire(kind, world){
    ...
    switch(kind){
      ...
      case 'gl': {
        // 低い放物線（必ず地面に到達）
        const vy = -260 + (Math.random()-0.5)*30;
        const vx = dir * w.speed * 0.45;
